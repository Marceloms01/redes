...
void repartir_carta(Jugador *jugador) {
    if (indice_mazo >= NUM_CARTAS) {
        printf("Mazo vacío, no se pueden repartir más cartas\n");
        return;
    }
    Carta carta = robarCarta(mazo, &indice_mazo);
...
void manejar_comando(int i, char* buffer) {
    ...
    // Comando: PEDIR-CARTA
       ...
       repartir_carta(jugador);

        char mensaje[MAX_BUFFER];
        Carta ultima_carta = jugador->mano[jugador->num_cartas - 1];
        sprintf(mensaje, "+Ok. Carta recibida: %d de %s. Puntuación: %d\n", ultima_carta.valor, ultima_carta.palo, jugador->puntuacion);
